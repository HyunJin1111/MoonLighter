...
HRESULT LightningJump::init(int skillMax, float range)
{
...
void LightningJump::release()
{
}

void LightningJump::update()
{
	move();
	_burn->update();
}

void LightningJump::render()
{
...
void LightningJump::fire(float x, float y, float angle)
{
	if (_SkillMax < _vSkill.size()) return;

	tagLightningJump Skill;
	ZeroMemory(&Skill, sizeof(tagLightningJump));
	_angle = angle;
	Skill.skillImage = new dImage;
	Skill.skillImage->init(L"skill/라이트닝.png", (int)410, 51, 16, 1);
	Skill.speed = 10.0f;
...
void LightningJump::move()
{
	for (_viSkill = _vSkill.begin(); _viSkill != _vSkill.end();)
	{
		...
		_viSkill->rc = RectMakeCenter(_viSkill->x, _viSkill->y - 20,
...
void LightningJump::removeSkill(int arrNum)
{
